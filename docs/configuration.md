...
| `HELIOAI_EXPERIMENTS` | — | Comma-separated names of the behaviours under evaluation, each off unless named: `deferred_tools` (the lead sees the formulary and catalogue tools only after asking with `search_tools`), `final_answer` (the lead may close with `final_answer(answer, claims)` and is asked to when it states measured numbers), `search_budget` (a role past its lookup allowance with nothing downloaded is told to use the ids it has), `search_variables` (the top hit of a parameter search lists its dataset's variables). An unknown name is refused when a model is built and by `helioai doctor` — a misspelt experiment must not be measured as if it ran. With `final_answer` off no claim verdict is produced. |

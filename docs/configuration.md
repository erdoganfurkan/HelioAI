...
| `HELIOAI_EXPERIMENTS` | — | Comma-separated names of the behaviours under evaluation, each off unless named: `deferred_tools` (the lead sees the formulary and catalogue tools only after asking with `search_tools`), `search_budget` (a role past its lookup allowance with nothing downloaded is told to use the ids it has), `search_variables` (the top hit of a parameter search lists its dataset's variables), `judgment_intent` (with `HELIOAI_JUDGMENT_BACKEND=jev`, the question is read once by the judge into an intent contract, placed against what the turn loaded and delivered — frame, window, quantity, responsiveness — and emitted as an `intent` event after the answer; observation, nothing in the loop acts on it). An unknown name is refused when a model is built and by `helioai doctor` — a misspelt experiment must not be measured as if it ran. `final_answer` graduated from this list after the third benchmark and is always on: the lead closes a measured answer with `final_answer(answer, claims)` and the claim verdict is produced. |

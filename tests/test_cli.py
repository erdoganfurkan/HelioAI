...
def test_show_history_with_sessions(capsys, monkeypatch):
    import helioai.core.session as s
    monkeypatch.setattr(s, "store", _make_store([
        {"session_id": "abc12345-rest", "updated_at": "2026-05-31T10:00:00Z",
         "n_messages": 4, "first_message": "solar wind density"},
    ]))
    from helioai.interfaces.cli import _show_history
    _show_history()
...
def test_pick_session_by_number(monkeypatch):
    summaries = [
        {"session_id": "session-aaa", "updated_at": "2026-05-31T10:00:00Z",
         "n_messages": 3, "first_message": "first query"},
        {"session_id": "session-bbb", "updated_at": "2026-05-30T09:00:00Z",
         "n_messages": 2, "first_message": "second query"},
    ]
    import helioai.core.session as s
...
def test_pick_session_by_prefix(monkeypatch):
    summaries = [{"session_id": "abc12345-0000", "updated_at": "2026-05-31T10:00:00Z",
                  "n_messages": 1, "first_message": "query"}]
    import helioai.core.session as s
...
def test_pick_session_skip_with_enter(monkeypatch):
    summaries = [{"session_id": "x", "updated_at": "2026-05-31T10:00:00Z",
                  "n_messages": 1, "first_message": "q"}]
    import helioai.core.session as s
...
def test_pick_session_no_match(monkeypatch):
    summaries = [{"session_id": "abc", "updated_at": "2026-05-31T10:00:00Z",
                  "n_messages": 1, "first_message": "q"}]
    import helioai.core.session as s
...
def test_main_resume_calls_interactive(monkeypatch):
    ...
    monkeypatch.setattr("helioai.interfaces.cli._interactive", lambda: interactive_calls.append(True))
